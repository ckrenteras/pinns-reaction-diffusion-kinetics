...
def plot_loss_history(results_dir=RESULTS_DIR, plots_dir=PLOTS_DIR):
    ...
    if 'phase' in df.columns and (df['phase'] == 'bfgs').any():
        bfgs_start = df.loc[df['phase'] == 'bfgs', 'step'].iloc[0]
        ax.axvline(bfgs_start, color='gray', linestyle=':', linewidth=1)
        ax.annotate('SSBroyden starts', xy=(bfgs_start, 1), xycoords=('data', 'axes fraction'),
                    xytext=(3, -3), textcoords='offset points', va='top', fontsize=8, color='gray')
    ax.set_xlabel('Step (1 pt/epoch during Adam, 1 pt/iteration during SSBroyden)')
    ax.set_ylabel('Loss')
    #ax.set_title('Per-Iteration Loss: Allen–Cahn PINN v3')
    ax.legend()
...
def plot_lambda_history(results_dir=RESULTS_DIR, plots_dir=PLOTS_DIR):
    ...
    ax.set_xlabel('Step (1 pt/epoch during Adam, 1 pt/iteration during BFGS)')
...
def plot_pointwise_error(plot_val, results_dir=RESULTS_DIR, plots_dir=PLOTS_DIR):
    ...
    #fig.suptitle('Pointwise Absolute Error: Allen–Cahn PINN v3', y=1.02)
    fig.tight_layout()
    os.makedirs(plots_dir, exist_ok=True)
