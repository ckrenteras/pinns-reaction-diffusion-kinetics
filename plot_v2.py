...
def plot_loss_history(results_dir=RESULTS_DIR, plots_dir=PLOTS_DIR):
    ...
    if 'phase' in df.columns and (df['phase'] == 'bfgs').any():
        ...
        ax.annotate('BFGS starts', xy=(bfgs_start, 1), xycoords=('data', 'axes fraction'),
    ...
    ax.set_ylabel('Loss')
    #ax.set_title('Per-Iteration Loss: Allen–Cahn PINN v2')
    ax.legend()
...
def plot_pointwise_error(plot_val, results_dir=RESULTS_DIR, plots_dir=PLOTS_DIR):
    ...
    #fig.suptitle('Pointwise Absolute Error: Allen–Cahn PINN v2', y=1.02)
    fig.tight_layout()
    os.makedirs(plots_dir, exist_ok=True)
